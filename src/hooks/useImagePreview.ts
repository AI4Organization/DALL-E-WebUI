...
export function useImagePreview(): UseImagePreviewReturn {
  ...
  const navigatePrevious = useCallback(() => {
    ...
    const newIndex = (previewState.currentIndex - 1 + previewState.totalImages) % previewState.totalImages;
  ...
  const navigateNext = useCallback(() => {
    if (!previewState) return;
    const newIndex = (previewState.currentIndex + 1) % previewState.totalImages;
  ...
  const toggleFullscreen = useCallback(() => {
    ...
    if (!document.fullscreenElement) {
      modalRef.current.requestFullscreen().catch(() => {
        // Fallback for browsers that don't support requestFullscreen on elements
